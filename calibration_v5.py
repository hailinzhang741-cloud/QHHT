...
from forecast_v5 import compute_probs, prepare_features
...
def optimize_and_save_thresholds(cal1: pd.DataFrame, cal5: pd.DataFrame, long_only: bool = True) -> dict:
    ...
    best5 = optimize_thresholds_v5(cal5, min_samples=8, long_only=long_only)
    return {
        "version": 5,
        "long_only": long_only,
        "filter": {
...
def eval_filter_v5(
    ...
    for _, r in cal_df.iterrows():
        ...
        bull, _ = count_agreement(probs, 5, extra_5d_keys=extra)
...
def optimize_thresholds_v5(
    ...
    for th_up in np.arange(0.55, 0.64, 0.01):
...
def format_calibration_report(m1: dict, m5: dict) -> str:
    lines = [
        ...
        "",
        "  v5: LME前日涨跌 + LME库存 + PMI；子模型 6 项；可回退至 v4。",
        "=" * 52,
    ]
    return "\n".join(lines)
